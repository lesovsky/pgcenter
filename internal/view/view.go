...
func New() Views {
	return map[string]View{
		"databases": {
			Name:      "databases",
			QueryTmpl: query.PgStatDatabaseQueryDefault,
		...
		"replication": {
			Name:      "replication",
			QueryTmpl: query.PgStatReplicationQueryDefault,
			DiffIntvl: [2]int{6, 6},
			Ncols:     15,
			OrderKey:  0,
			OrderDesc: true,
		...
		"tables": {
			Name:      "tables",
			QueryTmpl: query.PgStatTablesQueryDefault,
		...
		"indexes": {
			Name:      "indexes",
			QueryTmpl: query.PgStatIndexesQueryDefault,
			DiffIntvl: [2]int{1, 5},
			Ncols:     6,
			OrderKey:  0,
			OrderDesc: true,
		...
		"sizes": {
			Name:      "sizes",
			QueryTmpl: query.PgTablesSizesQueryDefault,
			DiffIntvl: [2]int{4, 6},
			Ncols:     7,
			OrderKey:  0,
			OrderDesc: true,
		...
		"functions": {
			Name:      "functions",
			QueryTmpl: query.PgStatFunctionsQueryDefault,
			DiffIntvl: [2]int{3, 3},
			Ncols:     8,
			OrderKey:  0,
			OrderDesc: true,
		...
		"progress_vacuum": {
			...
			QueryTmpl: query.PgStatProgressVacuumQueryDefault,
		...
		"progress_cluster": {
			...
			QueryTmpl: query.PgStatProgressClusterQueryDefault,
			DiffIntvl: [2]int{10, 11},
			Ncols:     13,
			OrderKey:  0,
			OrderDesc: true,
		...
		"progress_index": {
			Name:      "progress_index",
			QueryTmpl: query.PgStatProgressCreateIndexQueryDefault,
			DiffIntvl: [2]int{0, 0},
			Ncols:     14,
			OrderKey:  0,
			OrderDesc: true,
		...
		"statements_timings": {
			...
			QueryTmpl: query.PgStatStatementsTimingQueryDefault,
		...
		"statements_general": {
			...
			QueryTmpl: query.PgStatStatementsGeneralQueryDefault,
			DiffIntvl: [2]int{4, 5},
			Ncols:     8,
			OrderKey:  0,
			OrderDesc: true,
		...
		"statements_io": {
			Name:      "statements_io",
			QueryTmpl: query.PgStatStatementsIoQueryDefault,
			DiffIntvl: [2]int{6, 10},
			Ncols:     13,
			OrderKey:  0,
			OrderDesc: true,
		...
		"statements_temp": {
			...
			QueryTmpl: query.PgStatStatementsTempQueryDefault,
			DiffIntvl: [2]int{4, 6},
			Ncols:     9,
			OrderKey:  0,
			OrderDesc: true,
		...
		"statements_local": {
			...
			QueryTmpl: query.PgStatStatementsLocalQueryDefault,
			DiffIntvl: [2]int{6, 10},
			Ncols:     13,
			OrderKey:  0,
			OrderDesc: true,
...
func (v Views) Configure(version int, trackCommit string) {
	...
	for k, view := range v {
		...
		case "activity":
			switch {
			case version < 90600:
				view.QueryTmpl = query.PgStatActivityQuery95
				view.Ncols = 12
				v[k] = view
			case version < 100000:
				view.QueryTmpl = query.PgStatActivityQuery96
				view.Ncols = 13
				v[k] = view
			}
		case "replication":
			...
			case version < 90500:
				...
				view.QueryTmpl = query.PgStatReplicationQuery96
				view.Ncols = 12
				v[k] = view
			case version < 100000:
				// Check is 'track_commit_timestamp' enabled or not and use corresponding query for 9.6
				if track {
					view.QueryTmpl = query.PgStatReplicationQuery96Extended
					view.Ncols = 14
				} else {
					view.QueryTmpl = query.PgStatReplicationQuery96
					view.Ncols = 12
			...
			default:
				...
				if track {
					view.QueryTmpl = query.PgStatReplicationQueryExtended
					view.Ncols = 17
				} else {
		...
		case "databases":
			...
			case version < 120000:
				view.QueryTmpl = query.PgStatDatabaseQuery11
				view.Ncols = 17
		...
		case "statements_timings":
			switch {
			case version < 130000:
				view.QueryTmpl = query.PgStatStatementsTimingQuery12
				v[k] = view
			}
		}
	}
