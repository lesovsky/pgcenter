...
const (
	// PgStatProgressBasebackupDefault is the default query for getting stats from pg_stat_progress_basebackup view
	PgStatProgressBasebackupDefault = "SELECT " +
		...
		"p.backup_total / 1024 AS t_size," +
		`round(100 * p.backup_streamed / greatest(p.backup_total,1), 2)::text AS "streamed_%",` +
		"coalesce(p.backup_streamed / 1024, 0) AS streamed," +
