...
This task:
1. Extracts `buildProcPidResultRaw` (private) — assembles a 19-col PGresult where cols 6–11 hold raw float64 strings (jiffies for CPU, bytes for IO, ticks for iodelay), cols 0–5 are SQL labels, col 18 is query text.
2. Extracts `formatProcPidResultForDisplay` (private) — converts a raw PGresult to a display PGresult: cols 6–8 become HH:MM:SS, cols 9–10 become KiB integers (already integers in raw, just refornatted), col 11 becomes HH:MM:SS, cols 12–17 become float rate strings.
...
5. Defines `SysInfo` struct for use by the recorder (writes it to tar) and the reporter (reads it).
6. Updates all internal call sites of `getSysticksLocal` (in `stat.go`, `netdev_test.go`, `diskstats_test.go`, `stat_test.go`) to use `GetSysticksLocal`.
7. Adds unit tests for `buildProcPidResultRaw`, `formatProcPidResultForDisplay`, `GetSysticksLocal`, and `SysInfo` JSON round-trip.
...
3. In `internal/stat/procpidstat.go`, extract `buildProcPidResultRaw` from the body of `buildProcPidResult`:
   ...
   - Cols 12–17: rates computed exactly as today (float strings). These remain the same.
...
4. In `internal/stat/procpidstat.go`, extract `formatProcPidResultForDisplay` that takes the raw PGresult and returns a display PGresult:
   ...
   - Cols 12–17: pass through unchanged (already display strings).
   - All other cols: pass through unchanged.
   - `ticks` and `cpuCount` must be parameters since the function needs them for conversion.

5. Make `buildProcPidResult` a thin composition: call `buildProcPidResultRaw(...)`, then `formatProcPidResultForDisplay(rawResult, ticks)`, return the display result. The public signature of `buildProcPidResult` does not change.

6. In `internal/stat/stat.go`, rename `getSysticksLocal` → `GetSysticksLocal`. Update the call in `NewCollector` to use the new name.

7. In `internal/stat/netdev_test.go`, `internal/stat/diskstats_test.go`, and `internal/stat/stat_test.go`, replace all calls to `getSysticksLocal()` with `GetSysticksLocal()`.

8. Run all tests — verify all pass including the existing `TestBuildProcPidResult_*` suite.
...
- `internal/stat/procpidstat_test.go::TestFormatProcPidResultForDisplay` — verifies that `formatProcPidResultForDisplay` converts a known raw PGresult to a display PGresult: cols 6–8 contain HH:MM:SS strings (e.g. "00:00:01"), cols 9–10 are KiB integers (bytes/1024), col 11 is HH:MM:SS, cols 12–17 are float rate strings, col 18 is query text unchanged.
...
- [ ] `buildProcPidResult` public signature and behavior is unchanged — all existing `TestBuildProcPidResult_*` tests pass without modification
...
- [internal/stat/procpidstat.go](../../../../internal/stat/procpidstat.go) — extract `buildProcPidResultRaw`, `formatProcPidResultForDisplay`; add `SysInfo` struct; make `buildProcPidResult` a composition
...
`internal/stat/stat.go` — current state: `getSysticksLocal()` at line 372 (unexported). `NewCollector` at line 87 calls it. Change: rename to `GetSysticksLocal`. No signature change. The `// #nosec G204` comment on `exec.Command("getconf", "CLK_TCK")` should be preserved (gosec suppression for exec.Command with a literal string).
...
- The existing `TestBuildProcPidResult_*` tests do not call `buildProcPidResultRaw` directly — they only call `buildProcPidResult`. No existing test needs modification.
...
- The `gosec` suppression comment `// #nosec G204` on the `exec.Command` call in `getSysticksLocal` should be preserved verbatim when renaming to `GetSysticksLocal`.
