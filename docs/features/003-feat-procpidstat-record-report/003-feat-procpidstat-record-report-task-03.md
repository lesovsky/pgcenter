...
3. In `report/report.go`:
   ...
   f. In `processData` (or at the start of `doReport` before the goroutines), detect when no procpidstat entries were found in the archive after `readTar` completes. If the report type is `"procpidstat"` and no data rows were ever processed (`linesPrinted == 0` after `doneCh` closes), print `"INFO: no procpidstat data in this archive\n"` to `app.writer` and return nil. The simplest approach: check a counter — if no data pairs were ever processed after `doneCh` fires, emit the INFO message.
...
- `report/report_test.go::Test_readMeta_with_sysinfo` — **owned by Task 03; Task 04 does NOT own this test.** Build a synthetic in-memory tar containing: (1) a valid `meta.*` entry (standard 7-column PGresult with a version number) and (2) a `procpidstat.20260519T100000.000.json` stat entry with at least one data row, followed by (3) a `sysinfo.20260519T100000.000.json` entry containing `{"ticks":100,"cpu_count":4}`. The tar must include **both** a `meta.*` entry and a `procpidstat.*` stat entry — without both, `readTar`'s `metaOK && statOK` gate never fires and the channel goroutine hangs without sending any data. Call `readTar` with this tar (using a pipe or `bytes.Buffer`) and capture the `data` items sent on the channel. Assert that `meta.ticks == 100` and `meta.cpuCount == 4` in the received data item.

- `report/report_test.go::Test_processData_no_procpidstat_data` — call `processData` with a report type of `"procpidstat"` and an empty data channel (simulate a tar with no procpidstat entries). Assert that the function returns nil and that the writer receives the string `"INFO: no procpidstat data in this archive\n"`.
...
- `processData` INFO message (no-data path): after the `doneCh` channel closes (i.e., `readTar` has finished and no more data will arrive), check whether `linesPrinted == 0`. If the report type is `"procpidstat"` and `linesPrinted == 0`, write `"INFO: no procpidstat data in this archive\n"` to `app.writer` and return nil. This handles user-spec Scenario 5: running `pgcenter report -N` on a tar that was recorded without procpidstat (e.g., recorded in a version before this feature or with procpidstat disabled). The `linesPrinted` counter already exists in `processData` — no new variable needed. Check the counter only after the data channel is drained.
...
- **No procpidstat entries in archive (Scenario 5):** when a tar archive was recorded without procpidstat support (older version of pgcenter or disabled), `readTar` sends zero data items to the channel. After the channel drains, `linesPrinted == 0` and the INFO message is printed. The function returns nil — not an error — because the archive itself is valid; it simply has no procpidstat data. The test `Test_processData_no_procpidstat_data` covers this path.
