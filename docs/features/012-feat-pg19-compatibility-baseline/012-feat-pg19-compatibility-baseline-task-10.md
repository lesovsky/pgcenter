...
1. Confirm the environment is the published testing image from Task 8 (`lesovsky/pgcenter-testing:0.0.11`, or
   whatever tag Task 9 actually put into the workflows — read it from `.github/workflows/default.yml` rather
   than assuming), running clusters on ports 21914–21919. The `pre-deploy-qa` skill's Phase 1 ("rebuild the
   environment with `--no-cache`") maps here to: run the published image, verify all six clusters accept
   connections and the `pgcenter_fixtures` database exists on each. Do **not** rebuild the image from
   `testing/Dockerfile` — the published image is the artifact under test, and a local rebuild would silently
   pull different beta packages.
...
3. Run `./testing/e2e.sh` and confirm it covers port 21919 in both loops (record and report) — a green run
   that never touched 21919 proves nothing about PG 19.
4. Confirm the PG 19 subtests **executed** rather than skipped. `t.Skipf` on an unreachable cluster is the
   normal mechanism, so a green suite with a stopped 21919 cluster is also green — check the verbose output
   for actual PG 19 subtest execution.
...
6. One-off manual check: stop the 21919 cluster, run the suite, confirm PG 19 subtests **skip** rather than
   pass. Restart the cluster afterwards.
...
9. Run `pgcenter top` inside `tmux` against the live PG 19 cluster, switch screens by hotkey, and capture the
   pane after each switch into its own file. Every one of the 27 registered views must produce a capture —
   the full hotkey map is in Details.
10. The per-process screen (`S`) is only available on a **local** connection (`app.db.Local`; the host must be
    a unix socket path, `localhost`, `127.0.0.1`, `::1` or a local interface address). Run that part of the
    walk from inside the container so both the connection is local and `/proc` of the PostgreSQL backends is
    readable.
...
19. Produce the QA report per the `pre-deploy-qa` skill (`logs/working/qa-report.json`), covering every
    acceptance criterion from both the user-spec and the tech-spec, with evidence per criterion.
...
- [ ] `progress_vacuum` shows `started_by` and `mode`, `progress_analyze` shows `started_by`,
      `progress_basebackup` shows `backup_type`; values match `psql` at the same moment.
...
- [Makefile](../../../Makefile) — `test`, `lint`, `vuln` targets

## Verification Steps

- Run `make test`, `make lint`, `make vuln`, `./testing/e2e.sh` — all clean, e2e covers 21919.
- Confirm from verbose test output that PG 19 subtests executed against the live cluster.
...
`logs/working/qa-pg19-screens/<view-name>.txt`), and a decisions.md entry.

**Dependencies:** Task 9 (CI pointed at the published image, e2e script covering 21919), which in turn depends
on Task 8 (image published). Everything else in the feature is merged by the time this runs.
...
- **named explicitly in the QA report with its owning feature**, if the affected area is re-entered by a later
  feature of the same 0.12.0 release — [016] WAL and archiving, [017] tables and autovacuum, [018] replication
  and recovery, [019] locks. This is acceptable *only* because all of them ship in the same release, so no
  screen is broken at release time. It is re-verified at release finalization.
