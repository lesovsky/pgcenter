...
3. First-tick: признак первого тика — это **реальный флаг** (`verboseFirstTick`), который ввёл Task 7
   ...
   `nil`. Эта задача **группирует** Task-7-флаг в новый sub-struct `verboseCollectState` на `Collector`
   (рядом с per-source throttle/cadence-состоянием) и **завязывает на него** cmdline-подсказку
   `collecting...`: подсказка живёт, пока флаг первого тика выставлен, и **очищается после первого
   успешного refresh**. Подсказка живёт в `top/stat.go` (рядом с `printStat`/`printCmdline`), не
   вводит нового user-knob.
...
- В `internal/stat/stat.go`:
  - Объявить sub-struct `verboseCollectState` с полями для per-source cadence (момент последнего
    ...
    (stale) значения дорогого агрегата (db sizes / growth) и первотикового флага `verboseFirstTick`
    (который Task 7 ввёл отдельным полем на `Collector`). Встроить sub-struct в `Collector`,
    перенеся флаг Task 7 внутрь него; обновить точки выставления/сброса флага в collection-пути.
  ...
  - На первом verbose-тике дорогие/дельтовые источники дают `n/a` через существующую
    ...
    `collect*`-обёртки уже наполняют срез zero-delta через `prev = curr`). Здесь — лишь сохранить
    корректную работу этого флага после переезда в `verboseCollectState` и не ломать последующие тики.
...
- В `top/stat.go`:
  - Вывести в cmdline подсказку `collecting...`, завязанную на первотиковый флаг
    ...
    к нормальному сообщению вида). Не заводить отдельный дублирующий признак первого тика в `top/` —
    источник истины один, флаг из коллектора. Соблюсти mutual-exclusion `printCmdline` (см.
...
- `top/stat_test.go::Test_firstTickCollectingHint` — пока первотиковый флаг
  ...
  успешного refresh (флаг сброшен) подсказка очищается.

## Acceptance Criteria

- [ ] На `Collector` есть именованный sub-struct `verboseCollectState` с per-source cadence/latency,
      кешем последнего stale-значения дорогих агрегатов и первотиковым флагом `verboseFirstTick`
      (перенесён из Task 7); verbose-специфичные поля не размазаны по `Collector` россыпью.
...
- `internal/stat/stat.go` — текущее состояние: `Collector` (строки 51–74) хранит per-source prev/curr
  ...
  Task 7 — all-three system branch и первотиковый флаг `verboseFirstTick` (отдельным полем на
  `Collector`, заведён forward-compatible под эту перегруппировку). Что сделать: добавить sub-struct
  `verboseCollectState`, встроить в `Collector`, перенести флаг `verboseFirstTick` Task 7 внутрь
  него (обновив точки set/clear в collection-пути); объявить именованную latency-guard константу;
  обернуть **только** дорогой no-twin агрегат (db sizes / growth) троттлингом со stale-кешем;
  system-строки и прочие pgstat-агрегаты оставить каждый тик. `Reset()` (строки 111–119) — учесть:
  сброс при view-switch не должен оставлять «висящий» throttle-state, мешающий первому тику
  (сбрасывать/инициализировать `verboseCollectState`, включая `verboseFirstTick`, согласованно с
  prev/curr).
...
- Task 8 — composer'ы строк, читающие первотиковый флаг и рисующие `n/a` на первом кадре (рендер
  `n/a` уже есть; здесь флаг переезжает в `verboseCollectState`, плюс cmdline-подсказка на том же флаге).
