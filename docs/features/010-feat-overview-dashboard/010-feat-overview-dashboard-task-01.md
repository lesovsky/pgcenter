...
3. **Динамический суффикс единицы скорости.** Хелпер, форматирующий значение скорости в фиксированный
   ...
   делитель по источнику (как панель nicstat считает `Mbps` — это print-time конверсия `Rbytes/1024/128`
   в `printNetdev`, `top/stat.go:741`, а не поле структуры в `netdev.go`) и задокументировать в decisions.
...
- Делитель `MB/s→GB/s` = 1024 (бинарный, консистентно с `pretty.Size`). Для `Mbps→Gbps` зафиксируй
  делитель явно и обоснуй в decisions (панель nicstat оперирует Mbps; конверсия в `Mbps` происходит
  не в структуре `netdev.go`, а в момент печати — `printNetdev` делит `Rbytes/1024/128` в `top/stat.go:741`;
  сверься именно с этой строкой, чтобы verbose был консистентен с панелью nicstat).
