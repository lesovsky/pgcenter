...
4. Существующий `switch c.config.collectExtra` оставить полностью без изменений (R1).
5. Написать новый live-PG тест `TestCollector_Update_Verbose` в `internal/stat/stat_test.go` по паттерну
   `TestCollector_Update`, но с `view.Verbose = true` и проверкой, что все три — `Diskstats`, `Netdevs`,
   `Fsstats` — наполнены. Существующий `TestCollector_Update` и `TestCollector_collectDiskstats` должны
...
- `internal/stat/stat.go` — текущее состояние: `Update` (строки 122-289) после сбора CPU/mem/load делает
  ...
  ведут свой prev/curr и возвращают `(usage, error)`. Что сделать: вставить `if view.Verbose { ... }`
  сразу после строки 175 (закрытие switch), до `itv := ...` (178). `view` — параметр `Update`
  (`view view.View`), поле `view.Verbose` приходит из Task 2.
...
- Первый тик verbose: prev-снимок пустой, `count*Usage` возвращает `nil` при несовпадении длин — это валидно,
  Task 8 отрисует `n/a` (обрабатывается в Task 8, не здесь).
