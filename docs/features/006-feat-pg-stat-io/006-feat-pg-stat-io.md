...
- **Меню выбора под-экрана (`J`):** всплывающее меню из 2 пунктов —
  `pg_stat_io operations` / `pg_stat_io timings` (по образцу меню `pg_stat_statements`).
...
- Технический ключ для сопоставления строк между сэмплами (`io_key`) пользователю **не виден**
  (служебная колонка нулевой ширины); пользователь видит читаемые `backend_type/object/context`.
...
- `NULL` в счётчике (например, `fsyncs` для `temp relation`, `reads` для `background writer`) →
  значение приводится к 0 (`coalesce`), строка остаётся корректной; без этого один `NULL` внутри
  диапазона дельт обрушил бы весь сэмпл (пустой экран).
...
- [ ] PG 16/17: объём = `reads*op_bytes/1024` и т.д.; PG 18: нативные `*_bytes/1024` и присутствуют строки `object='wal'`. Ncols/DiffIntvl корректны per-branch (unit-тесты).
...
- [ ] Сопоставление строк между сэмплами по `io_key` даёт корректные дельты, когда ≥2 строк делят один `backend_type`.
...
- Ширина count-экрана ~165 колонок; на более узких терминалах хвостовые колонки обрезаются.
...
- **Риск 2 (средний):** ширина count-экрана ~165 колонок — на узких терминалах обрезается хвост.
  **Митигация:** осознанно принято; полный набор как у `statements`; колонки сортируемы для
  выбора релевантных; служебный `io_key` скрыт (экономит ширину).
...
- Скрываем строки с нулевыми операциями **в SQL** (`WHERE сумма_счётчиков > 0`) — так набор строк
  одинаков на обоих под-экранах.
...
| Селектор на каждой версии | go test (`io_test.go`) | Корректные `(Ncols, DiffIntvl)` для PG 16/17 и PG 18 |
| Diff со строкой `NULL` | go test | Сэмпл не обрушается, `NULL`→0 |
| `Test_selectMenuStyle` | go test (`menu_test.go`) | `menuStatIO` содержит 2 пункта |
