...
depends_on: ["02", "04"]           # ID задач-зависимостей (строки: ["01", "02"])
...
**Feature artifacts:**
- [010-feat-overview-dashboard.md](010-feat-overview-dashboard.md) — user-spec (см. «Состав и источники строк»: точная раскладка полей)
- [010-feat-overview-dashboard-tech-spec.md](010-feat-overview-dashboard-tech-spec.md) — tech-spec (Task 5, Decisions 6 и 10)
- [010-feat-overview-dashboard-code-research.md](010-feat-overview-dashboard-code-research.md) — code-research (§6-new: таблица per-row EXISTS/liftable/NET-NEW)
...
**Dependencies:** `depends_on: ["02", "04"]` (обе в wave 1; этот таск — wave 2).
- Task 2 (wave 1) — добавляет `view.View.Verbose bool`. Этот таск **гейтит сбор overview-агрегатов**
  на `view.Verbose` в `Collector.Update` (`if view.Verbose { ... }`), поэтому поле обязано существовать до
  старта — прямая блокирующая зависимость.
- Task 4 (wave 1) — GUC'и `GucMaxWorkerProcesses` / `GucMaxLogicalReplicationWorkers` / `GucMaxParallelWorkers`
  ...
  (лимиты воркеров, множитель backlog).
