...
Безопасность и устойчивость: первый verbose-тик → `n/a` для дельта-полей системных строк, не `0`. ВАЖНО:
первый тик НЕ определяется по пустому/`nil` слайсу `Diskstats`/`Netdevs` — `collectDiskstats`/`collectNetdevs`
при несовпадении длин делают `prev=curr` ДО `count*Usage`, поэтому первый verbose-тик отдаёт **заполненный
zero-delta слайс**, а не `nil`. First-tick определяется по явному first-tick-флагу, который коллектор
выставляет (добавлен в Task 7, далее сгруппирован в `verboseCollectState` Task 9) — ИМЕННО этот флаг гонит
`n/a`, а не `len(slice)==0`. По-настоящему простаивающее устройство с реальными нулевыми дельтами должно
показывать `0`, а не `n/a`. Недоступный источник → `n/a`, остальные строки рендерятся; любой сбой
`EvalSymlinks`/mount-match → `n/a`, не panic; сырой текст ошибок PG/путей в вывод не попадает.
...
- В `top/stat.go`, внутри `renderSysstat(w io.Writer, s stat.Stat, …)` (writer-ядро из Task 3), под флагом
  ...
  - **`iostat`**: выбрать устройство с максимальной `s.Diskstats[i].Util` среди устройств с `Completed != 0`
    ...
    На первом verbose-тике (first-tick-флаг коллектора выставлен) дельта-поля строки → `n/a`. НЕ опираться
    на `len(s.Diskstats)==0`: первый тик отдаёт заполненный zero-delta слайс, а не `nil`.
  - **`nicstat`**: выбрать интерфейс с максимальной `s.Netdevs[i].Utilization` среди интерфейсов с
    ...
    На первом verbose-тике (first-tick-флаг коллектора) дельта-поля → `n/a`; не по `len(s.Netdevs)==0`
    (первый тик отдаёт заполненный zero-delta слайс).
...
- `top/stat_test.go::Test_renderSysstat_verboseFirstTickNA` — при выставленном first-tick-флаге коллектора
  (а `s.Diskstats`/`s.Netdevs` уже **заполнены** zero-delta слайсом) строки `iostat`/`nicstat` рендерят `n/a`,
  не `0`. Контр-кейс: тот же заполненный слайс с реальными нулевыми дельтами, но БЕЗ first-tick-флага →
  показывает `0`, не `n/a` (флаг — единственный триггер `n/a`, не нулевые значения и не длина слайса).
...
- [ ] Первый verbose-тик → `n/a` (не `0`) по first-tick-флагу коллектора, НЕ по `len(slice)==0`
      (первый тик отдаёт заполненный zero-delta слайс); устройство с реальными нулевыми дельтами без флага
      показывает `0`. Недоступный источник → `n/a`, остальные строки целы.
...
**Edge cases:**
- Первый verbose-тик: `collectDiskstats`/`collectNetdevs` при несовпадении длин делают `prev=curr` ДО
  `count*Usage`, поэтому первый тик отдаёт **заполненный zero-delta слайс**, а НЕ `nil`. First-tick → `n/a`
  определяется по явному first-tick-флагу коллектора (Task 7 / `verboseCollectState` Task 9), НЕ по
  `len(s.Diskstats)==0`. По-настоящему idle-устройство с реальными нулевыми дельтами без флага → `0`, не `n/a`
  (Decision 5, risk-таблица tech-spec; task-reality batch3).
...
- `filesyst`: `data_directory` — это `props.DataDirectory` (GUC добавлен Task 4); сегодня в `PostgresProperties`
  его не было (читался ad hoc в `internal/stat/log.go:129`). Используй longest-prefix по компонентам пути.
