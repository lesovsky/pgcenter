...
- **sysinfo one-tick lag:** Spec described sysinfo as informational metadata read by report. Implementation has a one-tick lag: sysinfo written last in each tick is available only in metadata for the following tick (per the `metaOK && statOK` channel pairing in `readTar`). Under Option B (pass-through), sysinfo values don't affect output — rates are pre-computed. `Test_readMeta_with_sysinfo` uses a two-tick tar to demonstrate the merge contract correctly.

- **WARNING messages are two separate lines:** Spec showed a combined warning for IO + iodelay. Implementation emits two distinct WARNING lines (one for IO, one for iodelay) via `emitProcPidStatAvailabilityWarnings` helper, consistent with user-spec Scenario 4 requirements.

- **Two inline lint fixes in QA:** `record/record.go:102` (gosimple S1033 — redundant guard around delete) and `report/report.go:168` (gocritic ifElseChain — converted to switch) were fixed during pre-deploy QA gate. Both were style-only, introduced in Tasks 02/03.

### Added during implementation

- `emitProcPidStatAvailabilityWarnings` helper function in `report/report.go` with named col-index constants — not in spec, but cleaner than inline detection.
- `TestFilterViews_dropsExplicitNotRecordable` test (Task 04) — added to preserve coverage of the `NotRecordable=true` drop branch in `filterViews()` after procpidstat's flag was removed.
- `Test_describeReport` got a `procpidstat` case (Task 04 coverage gap from Task 03).
- `describe.go` — procpidstat description extracted to a separate file consistent with report package structure.

### Descoped / Deferred

- **AC11 (TUI Shift+S regression check):** Not auto-verifiable in QA shell (no TTY). Covered indirectly by `TestBuildProcPidResult_NcolsGuarantee` (19 cols, 6 sub-cases). Requires manual interactive check: `./bin/pgcenter top → Shift+S`.
- **Col-index constants export:** Local `procPidStatCol*` constants in `report/report.go` could be exported from `internal/stat/procpidstat.go`. Tracked in tech-debt [004].
