...
## Технические решения
- Мы решили строить экран по образцу `pg_stat_wal` (новый `internal/query/bgwriter.go` с
  `SelectStatBgwriterQuery(version) (string, int, [2]int)`), потому что это устоявшийся в проекте
  паттерн для однострочных версионно-зависимых вьюх.
